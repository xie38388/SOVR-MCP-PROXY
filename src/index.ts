...
// ============================================================================
// SOVR MCP Proxy wraps downstream MCP servers (Stripe, GitHub, etc.)
// Agent thinks it's calling Stripe directly, but SOVR intercepts:
//   Agent → SOVR Proxy → gate-check → forward to real Stripe → return result
...
// Proxy state
const downstreamServers: Map<string, DownstreamServer> = new Map();
const proxyToolMap: Map<string, string> = new Map(); // tool_name → server_name
...
function handleDownstreamData(server: DownstreamServer, chunk: string): void {
  ...
  while (true) {
    ...
    if (!clMatch) { server.buffer = server.buffer.substring(headerEnd + 4); continue; }
...
async function connectStdioDownstream(name: string, config: StdioDownstreamConfig): Promise<DownstreamServer> {
  ...
  proc.stdout!.on("data", (chunk: string) => handleDownstreamData(server, chunk));
...
function getProxyTools(): Array<{ name: string; description?: string; inputSchema?: unknown }> {
  ...
  for (const [, server] of downstreamServers) {
    ...
    for (const tool of server.tools) {
      // Return tools with original names — Agent doesn't know SOVR is in the middle
      tools.push(tool);
    }
  }
  return tools;
...
async function proxyToolCall(toolName: string, args: Record<string, unknown>): Promise<{ content: Array<{ type: string; text: string }>; isError?: boolean }> {
  ...
  // ── SOVR Gate Check ── Agent doesn't know this happens ──
  ...
  // ── Approved — forward to real downstream ──
...
// Cleanup on exit
process.on("SIGINT", () => { shutdownProxy(); process.exit(0); });
process.on("SIGTERM", () => { shutdownProxy(); process.exit(0); });
...
function evaluateCondition(cond: { field: string; operator: string; value: string }, context: Record<string, unknown>): boolean {
  ...
  switch (cond.operator) {
    ...
    case "regex": return new RegExp(cond.value, "i").test(actual);
        default: return false;
  }
}

const RISK_SCORES: Record<RiskLevel, number> = { none: 0, low: 20, medium: 50, high: 80, critical: 100 };

function evaluate(channel: Channel, action: string, resource: string, context: Record<string, unknown> = {}): EvalResult {
  ...
  const entry: AuditEntry = { decision_id: generateId(), timestamp: Date.now(), channel, action, resource, verdict, risk_score: riskScore, matched_rules: matchedRules.map((r) => r.id) };
...
interface ParsedCommand { command: string; subCommand: string | null; args: string[]; hasSudo: boolean; hasPipe: boolean; hasChain: boolean; riskIndicators: string[]; }
...
function parseCommand(raw: string): ParsedCommand {
  ...
  if (subCmdTools.includes(command) && parts[idx] && !parts[idx].startsWith("-")) { subCommand = parts[idx]; idx++; }
...
interface ParsedSQL { type: string; tables: string[]; hasWhereClause: boolean; isMultiStatement: boolean; raw: string; }

function parseSQL(raw: string): ParsedSQL {
  const trimmed = raw.trim();
  const isMultiStatement = trimmed.includes(";") && trimmed.replace(/;$/, "").includes(";");
  const typePatterns: [string, RegExp][] = [["DROP", /^DROP\b/i], ["TRUNCATE", /^TRUNCATE\b/i], ["ALTER", /^ALTER\b/i], ["CREATE", /^CREATE\b/i], ["DELETE", /^DELETE\b/i], ["UPDATE", /^UPDATE\b/i], ["INSERT", /^INSERT\b/i], ["SELECT", /^SELECT\b/i], ["GRANT", /^GRANT\b/i], ["REVOKE", /^REVOKE\b/i], ["EXEC", /^EXEC\b/i]];
  let type = "UNKNOWN";
  for (const [t, re] of typePatterns) { if (re.test(trimmed)) { type = t; break; } }
  const tables: string[] = [];
  ...
  for (const pattern of tablePatterns) { let match; while ((match = pattern.exec(trimmed)) !== null) { if (match[1] && !tables.includes(match[1])) tables.push(match[1]); } }
...
const DEFAULT_CLOUD_URL = "https://sovr-ai-mkzgqqeh.manus.space";
...
function tierHasAccess(userTier: Tier, toolName: string): boolean {
  return TIER_LEVEL[userTier] >= TIER_LEVEL[getToolTier(toolName)];
}

function filterToolsByTier<T extends { name: string }>(tools: T[], tier: Tier): T[] {
  return tools.filter(t => tierHasAccess(tier, t.name));
}

let currentTier: Tier = "free";

async function verifyKeyTier(): Promise<Tier> {
  ...
  try {
    ...
    const data = (await resp.json()) as { tier?: string; plan?: string };
    const rawTier = (data.tier || data.plan || "free").toLowerCase();
    const validTiers: Tier[] = ["free", "personal", "starter", "pro", "enterprise"];
    if (validTiers.includes(rawTier as Tier)) return rawTier as Tier;
    // If the Cloud returns a plan name that maps to a tier
    if (rawTier === "basic" || rawTier === "individual") return "personal";
    if (rawTier === "team" || rawTier === "business") return "pro";
    // If key exists but tier unknown, grant personal as minimum paid tier
    return "personal";
  } catch (err) {
    log(`[tier] Cloud verification error: ${err instanceof Error ? err.message : String(err)}`);
    // If we have a key but can't reach cloud, assume personal (paid) to avoid blocking
    return cloudApiKey ? "personal" : "free";
  }
}

interface CloudResponse { ok: boolean; data?: unknown; error?: string; }

const pendingCloudReqs: Promise<CloudResponse>[] = [];

async function cloudRequest(path: string, body: unknown, timeoutMs = 5_000): Promise<CloudResponse> {
...
async function cloudGet(path: string, timeoutMs = 5_000): Promise<CloudResponse> {
  if (!cloudEndpoint || !cloudApiKey) return { ok: false, error: "SOVR Cloud not configured" };
  try {
    const resp = await fetch(`${cloudEndpoint}${path}`, {
      method: "GET",
...
function cloudSyncFire(path: string, body: unknown): void {
  if (!cloudEndpoint || !cloudApiKey) return;
  const p = cloudRequest(path, body);
  pendingCloudReqs.push(p);
  p.finally(() => { const idx = pendingCloudReqs.indexOf(p); if (idx >= 0) pendingCloudReqs.splice(idx, 1); });
}

const hasCloud = (): boolean => !!cloudEndpoint && !!cloudApiKey;
...
interface McpToolDef { name: string; description: string; inputSchema: { type: "object"; properties: Record<string, unknown>; required?: string[] }; }


// ============================================================================
// SDK Route Map (1215 methods → HTTP routes)
// ============================================================================

interface SDKRoute { method: string; path: string; }
...
// ── Cloud Dispatch (routes any SDK method to Cloud API) ─────────────────────

async function cloudDispatch(toolName: string, operation: string, params: Record<string, unknown>): Promise<ToolResult> {
  if (!hasCloud()) {
    return fmt({
      status: "unavailable",
      tool: toolName,
  ...
  // Handle "list" operation — runtime discovery of available operations
  if (operation === "list") {
    const toolOps = Object.keys(SDK_ROUTES).filter(k => {
      // Simple heuristic: match operation names to tool names
  ...
  // Build the API path — route to correct backend based on path prefix
  // Core paths (bare paths like /gate-check, /replay-decision) → Cloud API (/api/sovr/v1/cloud)
  // Module paths (/p0/*, /p1p2/*, /p3/*, /p6/*, etc.) → MCP API (/api/mcp)
  const CLOUD_API_PATHS = new Set([
    '/gate-check', '/grant-permit', '/submit-receipt', '/replay-decision', '/export-bundle',
    '/status', '/request-approval', '/permit',
    '/openguard-scan', '/openguard-quick-scan', '/openguard-stats',
    '/role-request-stats',
    '/cloud-status', '/escalate', '/poll-escalation', '/decisions',
    '/violations', '/violation', '/violation-stats',
  ]);
  ...
  try {
    ...
    if (route.method === "GET") {
      ...
      const qs = Object.entries(params)
        ...
        .join("&");
      resp = await cloudGet(`${apiPath}${qs ? "?" + qs : ""}`, 15_000);
    } else {
      resp = await cloudRequest(apiPath, params, 15_000);
    }

    if (resp.ok) {
      return fmt({ status: "success", operation, ...(typeof resp.data === "object" && resp.data !== null ? resp.data as Record<string, unknown> : { data: resp.data }) });
    }
...
// ============================================================================
// MCP Tool Definitions (254 tools covering 1215 SDK methods)
// ============================================================================
...
function fmt(data: unknown): ToolResult {
...
async function handleToolCall(name: string, args: Record<string, unknown>): Promise<ToolResult> {
  switch (name) {
    ...
    case "sovr_gate_check": {
      const ch = (args.channel as Channel) ?? "mcp";
      const ctx = (args.context as Record<string, unknown>) ?? {};
      const d = evaluate(ch, args.action as string, args.resource as string, ctx);
      if (hasCloud()) {
        cloudSyncFire("/api/sovr/v1/cloud/gate", {
          decision_id: d.decision_id, verdict: d.verdict, risk_score: d.risk_score,
          channel: ch, action: args.action, matched_rules: d.matched_rules,
          reason: d.reason, parsed: ctx, timestamp: d.timestamp, mcp_server_version: VERSION,
        });
      }
    ...
    case "sovr_check_command": {
      const p = parseCommand(args.command as string);
      const actionKey = p.subCommand ? `${p.command}_${p.subCommand}` : p.command;
      const d = evaluate("exec", actionKey, p.args.join(" ") || "*", { command: p.command, args: p.args.join(" "), has_sudo: p.hasSudo, has_pipe: p.hasPipe, has_chain: p.hasChain, sub_command: p.subCommand ?? undefined, risk_indicators: p.riskIndicators, raw_command: (args.command as string).substring(0, 1000) });
      if (hasCloud()) { cloudSyncFire("/api/sovr/v1/cloud/gate", { decision_id: d.decision_id, verdict: d.verdict, risk_score: d.risk_score, channel: "exec", action: (args.command as string).substring(0, 128), matched_rules: d.matched_rules, reason: d.reason, parsed: { command: p.command, sub_command: p.subCommand, has_sudo: p.hasSudo }, timestamp: d.timestamp, mcp_server_version: VERSION }); }
      return fmt({ ...d, parsed: { command: p.command, sub_command: p.subCommand, has_sudo: p.hasSudo, risk_indicators: p.riskIndicators } });
    }

    case "sovr_check_sql": {
      const p = parseSQL(args.sql as string);
      const d = evaluate("sql", p.type, p.tables.join(",") || "*", { statement_type: p.type, tables: p.tables, has_where_clause: p.hasWhereClause, raw_sql: p.raw.substring(0, 1000), is_multi_statement: p.isMultiStatement });
      if (hasCloud()) { cloudSyncFire("/api/sovr/v1/cloud/gate", { decision_id: d.decision_id, verdict: d.verdict, risk_score: d.risk_score, channel: "sql", action: (args.sql as string).substring(0, 128), matched_rules: d.matched_rules, reason: d.reason, parsed: { type: p.type, tables: p.tables, has_where_clause: p.hasWhereClause }, timestamp: d.timestamp, mcp_server_version: VERSION }); }
    ...
    case "sovr_check_http": {
      ...
      let host: string;
      try { host = new URL(args.url as string).hostname; } catch { host = args.url as string; }
      const d = evaluate("http", method, host, { method, host, full_url: (args.url as string).substring(0, 500) });
      if (hasCloud()) { cloudSyncFire("/api/sovr/v1/cloud/gate", { decision_id: d.decision_id, verdict: d.verdict, risk_score: d.risk_score, channel: "http", action: `${method} ${host}`, matched_rules: d.matched_rules, reason: d.reason, parsed: { method, host }, timestamp: d.timestamp, mcp_server_version: VERSION }); }
      return fmt({ ...d, parsed: { method, host, full_url: args.url } });
    }

    case "sovr_request_approval": {
      if (hasCloud()) {
        const resp = await cloudRequest("/api/sovr/v1/cloud/approval", { action: args.action, resource: args.resource, reason: args.reason, urgency: args.urgency ?? "medium", channel: "mcp" }, 10_000);
        if (!resp.ok) return fmt({ status: "error", message: resp.error, fallback: "Cloud unavailable — logged locally only." });
        const ad = resp.data as { approval_id?: string; status?: string };
        ...
        while (Date.now() - start < 600_000) {
          ...
          try {
            const poll = await cloudRequest("/api/sovr/v1/cloud/approval", { approval_id: ad.approval_id, action: "check_status" }, 5_000);
            if (!poll.ok) continue;
            const pd = poll.data as { status?: string };
            if (pd.status === "approved") return fmt({ status: "approved", verdict: "allow", approval_id: ad.approval_id, message: "Action APPROVED. You may proceed.", elapsed_seconds: Math.round((Date.now() - start) / 1000) });
            if (pd.status === "rejected") return fmt({ status: "rejected", verdict: "deny", approval_id: ad.approval_id, message: "Action REJECTED. Do NOT proceed.", elapsed_seconds: Math.round((Date.now() - start) / 1000) });
    ...
    case "sovr_submit_receipt": {
      if (hasCloud()) {
        // Step 1: Obtain permit from grant-permit endpoint
        const permitResp = await cloudRequest("/api/sovr/v1/cloud/grant-permit", { decision_id: args.decision_id as string, ttl_seconds: 300 }, 10_000);
        if (!permitResp.ok) return fmt({ status: "error", message: `Failed to obtain permit: ${permitResp.error}` });
        const permitData = permitResp.data as { permit_id?: string };
        ...
        const now = Date.now();
        const outputSummary = (args.output_summary as string) || "";
        const externalRef = (args.external_ref as string) || `mcp-${now}`;
        const artifactRefs = (args.artifact_refs as string[]) || [];
        ...
        } catch { outputHash = `hash_${now}`; }
        // Step 3: Submit complete receipt with all required fields
        ...
        if (resp.ok) return fmt({ ...(resp.data as object), permit_id: permitData.permit_id, message: "Receipt submitted with permit and evidence hash." });
    ...
    case "sovr_add_rule": {
      const nr: PolicyRule = { id: args.id as string, description: args.description as string, channels: args.channels as Channel[], action_pattern: args.action_pattern as string, resource_pattern: args.resource_pattern as string, conditions: [], effect: args.effect as Verdict, risk_level: (args.risk_level as RiskLevel) ?? "medium", require_approval: args.effect !== "allow", priority: (args.priority as number) ?? 50, enabled: true };
      rules.push(nr);
    ...
    case "sovr_escalate": {
      return await cloudDispatch("sovr_escalate", "escalate", {
        ...
        reason: args.reason, context: args.context ?? {},
      });
    }
    ...
    case "sovr_query_decisions": {
      return await cloudDispatch("sovr_query_decisions", "queryDecisions", {
        limit: args.limit, verdict: args.verdict, action: args.action,
        from: args.from, to: args.to,
      });
    }
    case "sovr_query_violations": {
      return await cloudDispatch("sovr_query_violations", "queryViolations", {
        limit: args.limit, severity: args.severity, status: args.status,
      });
    }
    case "sovr_report_violation": {
      return await cloudDispatch("sovr_report_violation", "reportViolation", {
        action: args.action, resource: args.resource,
        violation_type: args.violation_type, severity: args.severity,
        details: args.details, decision_id: args.decision_id,
      });
    }
...
// ============================================================================
// MCP Protocol Handler (self-contained stdio transport)
// ============================================================================
  ...
  try {
    switch (method) {
      ...
      case "prompts/list":
        ...
        break;
      case "prompts/get": {
        const pn = (params as { name: string }).name;
        const pa = (params as { arguments?: Record<string, string> }).arguments;
      ...
      case "resources/read": {
        ...
        let rd: unknown;
        if (uri === "sovr://rules") { rd = rules.map((r) => ({ id: r.id, description: r.description, channels: r.channels, effect: r.effect, enabled: r.enabled })); }
        else if (uri === "sovr://audit-log") { rd = auditLog.slice(0, 50); }
        else if (uri === "sovr://status") { const visibleTools = filterToolsByTier(TOOLS, currentTier).length; rd = { mode: hasCloud() ? "cloud" : "local", version: VERSION, tier: currentTier, rules_count: rules.length, audit_count: auditLog.length, tools_available: visibleTools, cloud_connected: hasCloud(), proxy: { enabled: proxyEnabled, downstream_servers: downstreamServers.size, intercepted_tools: proxyToolMap.size } }; }
        else { sendResponse({ jsonrpc: "2.0", id, error: { code: -32602, message: `Unknown resource: ${uri}` } }); break; }
        sendResponse({ jsonrpc: "2.0", id, result: { contents: [{ uri, mimeType: "application/json", text: JSON.stringify(rd, null, 2) }] } });
        break;
      }
      case "tools/call": {
        const tn = (params as { name: string }).name;
        const ta = (params as { arguments?: Record<string, unknown> }).arguments ?? {};
        try {
          // Tier access check — block tools above current tier
          ...
          // Check if this is a proxied downstream tool
...
function startStdioTransport(): void {
  ...
  process.stdin.on("data", (chunk: string) => {
    ...
    while (true) {
      ...
      if (headerEnd === -1) break; // Wait for complete header
      const header = buffer.substring(0, headerEnd);
      const clMatch = header.match(/Content-Length:\s*(\d+)/i);
      if (!clMatch) { buffer = buffer.substring(headerEnd + 4); continue; }
      const cl = parseInt(clMatch[1], 10);
      const bodyStart = headerEnd + 4;
      if (buffer.length < bodyStart + cl) break; // Wait for complete body
      const body = buffer.substring(bodyStart, bodyStart + cl);
      buffer = buffer.substring(bodyStart + cl);
      try { handleRequest(JSON.parse(body) as JsonRpcRequest); } catch (err) { log(`Parse error: ${err}`); }
    }
  });
  process.stdin.on("end", async () => {
    log("Connection closed");
    if (pendingCloudReqs.length > 0) { log(`Flushing ${pendingCloudReqs.length} pending cloud sync(s)...`); await Promise.allSettled(pendingCloudReqs).catch(() => {}); }
    process.exit(0);
  });
}
...
`);
  ...
  if (process.env.SOVR_RULES_FILE) {
    try {
      ...
      // Support both { rules: [...] } wrapper format and direct [...] array format
      const customRules: PolicyRule[] = Array.isArray(raw) ? raw : (Array.isArray(raw?.rules) ? raw.rules : []);
      for (const r of customRules) {
        rules.push({
          ...
          effect: r.effect as Verdict,
          risk_level: (r.risk_level as RiskLevel) || "medium",
  ...
  // Verify API key tier at startup
  if (hasCloud()) {
...
const isMain =
  ...
  process.argv[1] &&
  (process.argv[1].includes("sovr-mcp-server") ||
    process.argv[1].includes("sovr-mcp-proxy") ||
    process.argv[1].endsWith("index.js") ||
    process.argv[1].endsWith("index.mjs") ||
    process.argv[1].endsWith("index.cjs"));

if (isMain) {
  // If --upstream flag is present, use the programmable proxy CLI
...
// ============================================================================
// McpProxy — Programmable Proxy API (merged from @sovr/proxy-mcp)
// ============================================================================
// Import-friendly class for embedding SOVR proxy in custom code.
// Uses the built-in policy engine — zero external dependencies.
//
// Usage:
//   import { McpProxy } from 'sovr-mcp-proxy';
//   const proxy = new McpProxy({
...
export interface SingleUpstreamConfig {
...
export interface EscalatedCallInfo {
  method: string;
  toolName: string;
  arguments: Record<string, unknown>;
  decision: EvalResult;
...
/** JSON-RPC 2.0 message types for proxy */
interface ProxyJsonRpcRequest {
  jsonrpc: '2.0';
  id?: string | number;
  method: string;
...
interface ProxyJsonRpcResponse {
  jsonrpc: '2.0';
  id?: string | number;
  result?: unknown;
  error?: {
...
export class McpProxy extends EventEmitter {
  private upstreamConfig: SingleUpstreamConfig;
  private upstream: ChildProcess | null = null;
  private _serverName: string;
  private _verbose: boolean;
  private _onBlocked?: McpProxyConfig['onBlocked'];
  private _onEscalated?: McpProxyConfig['onEscalated'];
  private _onIntercept?: McpProxyConfig['onIntercept'];
  private _stats: ProxyStats;
  private _customRules: PolicyRule[];
  ...
  constructor(config: McpProxyConfig) {
    ...
    // Merge custom rules into the engine
  ...
  async start(): Promise<void> {
    ...
    this.upstream.stderr?.on('data', (data: Buffer) => {
      if (this._verbose) {
    ...
    // Read requests from agent (stdin)
  ...
  private handleAgentMessage(line: string): void {
    let msg: ProxyJsonRpcRequest;
    try {
      msg = JSON.parse(line) as ProxyJsonRpcRequest;
    } catch {
  ...
  private interceptToolCall(request: ProxyJsonRpcRequest): void {
    ...
    // Extract danger signals for better rule matching
    const dangerSignals = this.extractDangerSignals(toolName, toolArgs);

    // Evaluate against built-in policy engine
  ...
  private sendBlockedResponse(request: ProxyJsonRpcRequest, decision: EvalResult): void {
    const response: ProxyJsonRpcResponse = {
      jsonrpc: '2.0',
      id: request.id,
      error: {
        code: -32001,
  ...
  private sendEscalatedResponse(request: ProxyJsonRpcRequest, decision: EvalResult): void {
    const response: ProxyJsonRpcResponse = {
      jsonrpc: '2.0',
      id: request.id,
      error: {
  ...
  private forwardToUpstream(data: string): void {
 ...
 */
export async function proxyCli(args: string[]): Promise<void> {
  let upstreamCmd = '';
  ...
  if (!upstreamCmd) {
    // No --upstream flag: fall through to normal MCP server mode
    return main();
  }
...
export {
  ...
  filterToolsByTier,
  tierHasAccess,
  // Multi-downstream proxy exports
  initProxy,
  shutdownProxy,
  proxyEnabled,
  downstreamServers,
